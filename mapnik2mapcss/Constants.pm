...
# The expected linejoin types - translates the corresponding MapCSS values.
use constant LINEJOIN_TYPES => {
    'miter' => 'miter', 
    'round' => 'round', 
    'bevel' => 'bevel'
};

# The expected linecap types - translates the corresponding MapCSS values.
use constant LINECAP_TYPES => {
    'round' => 'round', 
    'butt' => 'none', 
    'square' => 'square'
};
...
# Note that black maps to 0, so use 'exists CSS_COLORS->{$clr}' to check if a color is in the list of keys.
use constant CSS_COLORS => {
    ...
    'goldenrod' => 0xdaa520, 'gray' => 0x808080, 'green' => 0x008000, 'greenyellow' => 0xadff2f, 'grey' => 0x808080, 'honeydew' => 0xf0fff0, 
    ...
    'springgreen' => 0x00ff7f, 'steelblue' => 0x4682b4, 'tan' => 0xd2b48c, 'teal' => 0x008080, 'thistle' => 0xd8bfd8, 'tomato' => 0xff6347, 
    'turquoise' => 0x40e0d0, 'violet' => 0xee82ee, 'wheat' => 0xf5deb3, 'white' => 0xffffff, 'whitesmoke' => 0xf5f5f5, 'yellow' => 0xffff00, 
...
{
    my ($key, $value);
    my $colors = CSS_COLORS;
    while (($key, $value) = each %$colors) {
        $color_back{$value} = $key;
    }
}

# reverse lookup for CSS_COLORS
sub color_from_code {
    my $code = shift;
    my $result = $color_back{$code};
    if (defined $result) 
    {
        return $result;
    }
